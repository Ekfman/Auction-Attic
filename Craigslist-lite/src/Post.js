        ...
        <div className="postCard" key={post._id}>
          ...
          <div className="postBody">
            ...
            <p>Posted: {post.createdAt}</p>
                  ...
                  {post.isAuthor && <button onClick={ () => handleEditButton()} className="buttonForm">{editFormDisplay ? "Cancel" : "Edit"}</button>}
                  <div className="messageButton">
            {!post.isAuthor && 
            <button onClick={ () => handleMessageButton()} className="buttonForm">{messageFormDisplay ? "Cancel" : "Message"}</button>}
            </div>
                   { post.isAuthor && 
            ...
            <div>
              {token && messageFormDisplay && <MessageForm postId={post._id} token={token} handleMessageSent={handleMessageSent} username={post.author.username}/> }
              </div>
          </div>
        </div>
      );
